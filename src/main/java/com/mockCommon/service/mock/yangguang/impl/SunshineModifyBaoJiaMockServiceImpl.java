...
import com.mockCommon.model.mock.yangguang.SunshineModifyBaoJiaMockModel;
...
@Service("sunshineModifyBaoJiaMockServiceImpl")
public class SunshineModifyBaoJiaMockServiceImpl implements
	...
	@Autowired
	private IDataMaker<SunshineModifyBaoJiaMockModel> sunshineModifyBaoJiaFreeMarker;
	...
	public String modifyBaoJia(String xml) {
		...
		if (StringUtils.isEmpty(xml)) {
			LogConstant.debugLog
					.error("[SunshineModifyBaoJiaMockServiceImpl-->modifyBaoJia] xml can not be empty!");
			return result;
		}
		...
		if (xmlParams == null) {
			LogConstant.debugLog
					.error("[SunshineModifyBaoJiaMockServiceImpl-->modifyBaoJia] parse xml parameters return empty!");
			return result;
		}
		...
		String cov_390 = xmlParams.get("cov_390");
		...
		SunshineModifyBaoJiaMockModel sunshineModifyBaoJiaMockModel = new SunshineModifyBaoJiaMockModel();
		...
		if (!StringUtil.isEmpty(cov_390)) {
			sunshineModifyBaoJiaMockModel.setCov390Value(cov_390);
			if ("-1".equals(sunshineBaoJiaModel.getCov_390())) {
				sunshineModifyBaoJiaMockModel.setCov390Data("不可投保:0");
				sunshineBaoJia.setCov_390("-1");
			} else {
				sunshineBaoJia.setCov_390(cov_390);
				if(!cov_390.equals("0")){
					BusinessIni ini = iniMap.get("cov_390");
					if (ini != null) {
						String cov_390Baofei = ini.getIniValue();
						baoJiaModel.setCov_390(cov_390Baofei);
						bizTotalPremium += Double.parseDouble(cov_390Baofei);
					}
				}else{
					baoJiaModel.setCov_390("0");
				}
				sunshineModifyBaoJiaMockModel.setCov390Data("不投保:0;投保:1");
			}
		} else {
			LogConstant.debugLog.info("阳光保险修改报价接口[cov_390]请求值为空！");
			return result;
		}
		...
		// 生成最终返回数据
		String packageBody = sunshineModifyBaoJiaFreeMarker
				.generateData2output(
						ContextConstant.PREFIX_SUNSHINE_MODIFY_BAOJIA,
						sunshineModifyBaoJiaMockModel);
